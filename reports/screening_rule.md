...
本研究采用“**检索 → 筛选 → 数据提取 → 文献计量分析**”的流程，对**时序预测（time series forecasting）方法**在电力负荷预测（power load forecasting）中的研究进行系统分析。
...
- 各类时序预测方法（time series forecasting methods），包括：
  - 深度学习模型（如 LSTM、GRU、Transformer 等）
  - 传统机器学习方法（如 SVR、Random Forest 等）
  - 统计模型（如 ARIMA、SARIMA 等）
- 不同方法在电力负荷预测中的应用
- 时间范围：2015–2025 年
...
- 使用任意**时序预测方法（time series forecasting methods）**
...
- 是否使用**时序预测方法关键词**（如 LSTM, GRU, Transformer, ARIMA, forecasting 等）
...
- 关键词（keywords）
- 模型类型（model）
- 方法类别（method category，如 DL / ML / statistical）
- 数据集（dataset）
- 评价指标（metrics）
- 引用次数（citation）
