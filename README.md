...
基于复筛后数据，更新并完善指标规范文档 metrics_spec.md，明确发文趋势、被引分布等核心计量指标的计算口径。完成指标可视化分析，生成发文趋势、被引分布等结果文件（*.csv / *.png），相关处理脚本上传至 src/ 目录。迭代更新全局参数配置 params.md，记录本次分析使用的所有参数版本。
...
基于复筛后数据与新选题，更新 data_model.md、data_quality.md 与 cleaning_rules.md，保留历史版本并补充新内容。绘制图数据模型草图，明确节点、边、属性定义及示例数据。相关数据处理代码上传至 src/ 目录，确保流程可复现。
...
基于新数据改进共被引网络构建脚本 co_citation.py，优化网络生成与中心性计算逻辑。将共被引网络的关键参数写入 params.md（保留历史版本，在文档末尾补充更新内容）。完成基线版本配置，编写 baseline/params.md，作为后续分析的对照基准。
...
在共被引网络完成后，改进耦合 / 合作网络脚本 coupling_or_collab.py，实现文献耦合与作者 / 机构合作网络构建。更新 cleaning_rules.md，补充作者 / 机构消歧规则与版本信息。将耦合 / 合作网络参数写入 params.md，并完成工具路线对照表 tool_selection.md 的编写，明确项目使用的工具与方法路线。
